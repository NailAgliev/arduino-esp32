...
BLEServer::BLEServer()
{
	...
	m_connectedServersMap = *(new std::map<uint16_t, conn_status_t>());
} // BLEServer
...
void BLEServer::handleGATTServerEvent(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
	...
	case ESP_GATTS_CONNECT_EVT:
	{
		...
		for (auto elem : BLEDevice::getPeerDevices(true))
		{
			if (static_cast<uint16_t>(elem.first) == m_connId + 1) //realy dirty hack
			{
				goto exit;
			}
		}
	...
	{
		m_connId = param->disconnect.conn_id;
		if (!getPeerDevices(true).empty())
		{
			for (auto elem : getPeerDevices(true))
			{
				if (static_cast<uint16_t>(elem.first) != m_connId) //realy dirty hack
				{
					goto exitt;
				}
			}
