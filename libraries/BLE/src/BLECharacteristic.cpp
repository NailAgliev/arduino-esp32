...
void BLECharacteristic::notify(bool is_notification)
{
	...
	{
		log_e("<< notify: No connected clients.");
	...
	else
	{
		...
		{
			log_e("<< indications disabled; ignoring");
	...
	for (auto &myPair : getService()->getServer()->getPeerDevices(false))
	{
		...
		if (!BLEDevice::getPeerDevices(true).empty() ? free_buff_num > 5 : true) 
		{
		...
		else
		{
			...
			m_pCallbacks->onStatus(this, BLECharacteristicCallbacks::Status::ERROR_GATT, 0); // Invoke the notify callback.
			return;
		}
...
void BLECharacteristic::setCallbacks(BLECharacteristicCallbacks *pCallbacks)
{
	...
	{
		if (m_pCallbacks != &defaultCallback)
		{
			delete m_pCallbacks;
		}
